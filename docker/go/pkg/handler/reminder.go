...
func GetRemainingTime(now time.Time, reminder domain.Reminder) (time.Duration, error) {
	...
	for {
		remindDay = (remindDay + 1) % 7
		if reminder.When.Week[remindDay] {
			break
		}
	}
	d := AddDays(todayDay, remindDay)
	// mtn, avant, après
	nextReminder := time.Date(now.Year(), now.Month(), now.Day(), reminder.From, 0, 0, 0, now.Location()).Add(d)

	if now == nextReminder {
		nextReminder = nextReminder.Add(168 * time.Hour)
	}
